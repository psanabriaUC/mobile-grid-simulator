...
public class MyEdgeStealerProxy extends StealerProxy {
    private HashMap<String, Long> assignedJobs;
    private DeviceComparator initialComparator;

    public MyEdgeStealerProxy(String name) {
        super(name);
    ...
    protected void assignJob(Job job) {
        if (this.devices.isEmpty()) {
            ...
            for (Device device : this.devices.values()) {
                if (!device.runsOnBattery() && (selectedDevice == null || initialComparator.compare(device, selectedDevice) <= 0)) {
