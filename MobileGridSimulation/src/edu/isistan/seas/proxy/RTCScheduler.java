...
public class RTCScheduler extends DataIntensiveScheduler {
	...
	protected void assignJob(Job job) {
		...
		InputTransferInfo transferInfo = new InputTransferInfo(current, job, subMessagesCount, 0, lastMessageSize);
		...
		if(!lastPendingTransfers.containsKey(current))
		{
			...
			InputTransferInfo lastTInfo = lastPendingTransfers.get(current);
